...
class TestTrackImageAPI:
    ...
    def test_set_track_image(self, mock_exists, player):
        ...
        with mock.patch.object(
            player, "_is_format_supported", return_value=True
        ):
            player.add_to_playlist("track.mp3")

        # Set image for the track
        with mock.patch.object(
            player, "_is_image_format_supported", return_value=True
        ):
        ...
        assert player._track_images[track_path] == str(
            Path("cover.jpg").absolute()
        )

        # Check that playlist entry was updated
        assert player.playlist[0].get("image") == str(
            Path("cover.jpg").absolute()
        )
    ...
    def test_invalid_image_format(self, mock_exists, player):
        ...
        with mock.patch.object(
            player, "_is_image_format_supported", return_value=False
        ):
    ...
    def test_nonexistent_track_or_image(self, mock_exists, player):
        ...
        mock_exists.side_effect = [
            False,
            True,
        ]  # track doesn't exist, image exists
        ...
        mock_exists.side_effect = [
            True,
            False,
        ]  # track exists, image doesn't exist
        with pytest.raises(FileNotFoundError):
